...
try:
 with sync_playwright() as p:
  ...
  page.wait_for_function("document.querySelector('#health').textContent.includes('проверены')")
  checks.append('real_local_navigation')
  page.select_option('#model','compact4');page.click('#example')
  page.wait_for_function("document.querySelector('#age').value==='55'")
  page.click('#predict');page.wait_for_function("document.querySelector('#downloadJSON').disabled===false")
  ...
  page.select_option('#country','RU');page.click('#predict');page.wait_for_function("document.querySelector('#message').textContent.includes('не выполнены')")
  assert page.locator('.risk').count()==0
  checks.append('Russia_blocked_no_probabilities')
  page.select_option('#model','full8');page.click('#example');page.wait_for_function("document.querySelector('#age').value==='55'")
  page.click('#compare');page.wait_for_function("document.querySelectorAll('.prediction').length===4")
  ...
  page.fill('#jsonInput',json.dumps(payload));page.click('#predict');page.wait_for_function("document.querySelectorAll('.prediction').length===1")
