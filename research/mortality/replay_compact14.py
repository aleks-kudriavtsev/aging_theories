"""Explicit compact4 research replay. Standard library; no runtime fitting.

Late-cycle evaluation validates only three coarse death groups, not the five
latent hazards individually. A complete compact profile is required; the direct
comparison used participants who also had complete full8 profiles.
"""
...
MODEL_SHA='274753ea36728652d10b10fa7d02b6f77b37304084bb2b0b056128b734e394d4'
MODEL_PATH=Path(__file__).parent/'compact14/model_compact14.json'
GROUPS=('heart_diseases','malignant_neoplasms','chronic_lower_respiratory','cerebrovascular','other_or_unknown')
REQUIRED=('bmi','sbp','hba1c','creatinine','uacr','albumin')
HORIZONS=(1,3)


def load_bundle(path=MODEL_PATH):
    raw=Path(path).read_bytes()
    if hashlib.sha256(raw).hexdigest()!=MODEL_SHA:raise ValueError('Compact model checksum mismatch')
    b=strict_json(raw)
    if b.get('groups')!=list(GROUPS) or b.get('clinical_use_ready') is not False:
        raise ValueError('Invalid compact model scope')
    return b


def example():
    p=legacy_example();p.pop('model')
    p['measurements']={k:v for k,v in p['measurements'].items() if k in REQUIRED}
    p.update(schema_version='0.14.0',panel='compact4',endpoint='three_coarse_death_groups',horizons_years=[1,3])
    return p
...
def normalize(payload):
    ...
    for key,value in [('schema_version','0.14.0'),('endpoint','three_coarse_death_groups')]:
        if p.get(key)!=value:errors.append({'field':key,'code':'unsupported_definition'})
    panel=p.get('panel')
    if not isinstance(panel,str) or panel not in ('compact4',):errors.append({'field':'panel','code':'explicit_panel_required'})
    ...
    if (not isinstance(h,list) or not h or len(h)>3 or any(not finite(v) or v not in HORIZONS for v in h)
        ...
        errors.append({'field':'horizons_years','code':'only_unique_1_3_years'})
    ...
    for key in set(m)-set(REQUIRED):
        errors.append({'field':'measurements.'+key,'code':'not_in_compact_profile'})
...
def score_engineered(features, model, horizons):
    ...
    for h in horizons:
        ...
        result.append({'horizon_years':h,'survival':s,'cause_specific_cif':{'heart_diseases':cif[0],'malignant_neoplasms':cif[1],'other_or_unknown':math.fsum(cif[2:])},
...
def run(payload,*,bundle_path=MODEL_PATH):
    ...
    r={'version':'0.14.0','status':'blocked' if errors else 'calculated_research_only','clinical_use_ready':False,
       'model_sha256':MODEL_SHA,'endpoint':'three_coarse_death_groups','probabilities':None,
       'noninfectious_natural_probability':None,'individual_uncertainty_interval':None,
       'imputation_applied':False,'fit_on_request':False,'errors':errors,'warnings':warnings,
       'measurement_trace':trace,'clinical_history_and_physiology_required':True,
       'reported_lab_measures':4,'minimum_underlying_lab_determinations':5,
       'disclaimer':'Исследовательский прогноз исторической общей смертности США, 40–79 лет, 1/3 года. '
          'Остаточная группа включает инфекции и внешние причины. Не для клинических решений. '
          'Прямое сравнение панелей выполнено на участниках с полным расширенным профилем; '
          'новая страна, метод анализов и популяция требуют отдельной проверки.'}
    if errors:return r
    f=engineer(payload['age_years'],payload['sex'],payload['clinical'],values)
    r['probabilities'],r['feature_trace']=score_engineered(f,bundle['model'],payload['horizons_years'])
    r['panel']='compact4'
    r['declaration']='synthetic_not_a_person' if payload['dataset_context']=='synthetic_example' else 'historical_research'
    r['warnings'].append({'code':'compact_input_coverage_is_not_validation_of_additional_patients'})
    return r
